...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const userGenderSelect = document.getElementById('user_gander');
    ...
    function validateForm(event) {
        ...
        if (!userNameInput.value.trim()) {
            document.getElementById('user_name_err').textContent = 'براہ مہربانی اپنا نام درج کریں۔';
            userNameInput.classList.add('is-invalid');
            isValid = false;
        }

        if (userGenderSelect.value === '') {
            document.getElementById('user_gander_err').textContent = 'براہ کرم جنس منتخب کریں۔';
            userGenderSelect.classList.add('is-invalid');
            isValid = false;
        }

        if (!userAgeInput.value.trim()) {
            document.getElementById('user_age_err').textContent = 'براہ کرم تاریخ پیدائش درج کریں۔';
            userAgeInput.classList.add('is-invalid');
            isValid = false;
        }

        if (!userPhoneInput.value.trim()) {
            document.getElementById('user_phone_err').textContent = 'براہ کرم فون نمبر درج کریں۔';
            userPhoneInput.classList.add('is-invalid');
            isValid = false;
        }

        if (!userAddressInput.value.trim()) {
            document.getElementById('user_address_err').textContent = 'براہ کرم پتہ درج کریں۔';
            userAddressInput.classList.add('is-invalid');
            isValid = false;
        }

        if (userRoleSelect.value === '') {
            document.getElementById('user_role_err').textContent = 'براہ کرم یوزَر منتخب کریں۔';
            userRoleSelect.classList.add('is-invalid');
            isValid = false;
        }

        if (!usernameInput.value.trim()) {
            document.getElementById('username_err').textContent = 'براہ کرم یوزَر نام درج کریں۔';
            usernameInput.classList.add('is-invalid');
            isValid = false;
        }

        if (!userEmailInput.value.trim()) {
            document.getElementById('user_email_err').textContent = 'براہ کرم ای میل درج کریں۔';
            userEmailInput.classList.add('is-invalid');
            isValid = false;
        }

        if (!userPasswordInput.value.trim()) {
            document.getElementById('user_password_err').textContent = 'براہ کرم پاس ورڈ درج کریں۔';
    ...
    userGenderSelect.addEventListener('change', function () {
        document.getElementById('user_gander_err').textContent = '';
        userGenderSelect.classList.remove('is-invalid');
    });

    userAgeInput.addEventListener('input', function () {
        document.getElementById('user_age_err').textContent = '';
    ...
    userAddressInput.addEventListener('input', function () {
        document.getElementById('user_address_err').textContent = '';
        userAddressInput.classList.remove('is-invalid');
    });

    userRoleSelect.addEventListener('change', function () {
        document.getElementById('user_role_err').textContent = '';
